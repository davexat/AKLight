...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        snprintf(message, sizeof(message), "PUB %s/%s %s", base_topic, metric_names[metric1], metric_value);
        ...
        snprintf(message, sizeof(message), "PUB %s/%s %s", base_topic, metric_names[metric2], metric_value);
        send_string(socket_fd, message);
